            ...
            fetch('/delivery/details/'+deliveryId).then(r=>r.json()).then(data=>{
                ...
                if(data.header.status==='Pending'){
                    ...
                    fetch('/delivery/remaining/'+data.header.pr_id).then(r=>r.json()).then(rem=>{
                        ...
                        if(remainingTotal > 0){
                            approveArea.innerHTML+=`<button type="button" class="btn-modal-cancel" style="background:#fff8e1;border:1px solid #ffe082;margin-left:8px;" onclick="closeViewModal(); openCompleteModal(${data.header.delivery_id});"><svg class="act-icon" aria-hidden="true"><use href="#i-plus"/></svg> Add Remaining (${remainingTotal} left)</button>`;
                        }
                    });
                    }
                }
