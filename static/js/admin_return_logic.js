/* admin_return_logic.js — return modal logic. Loaded after ui_helpers.js. */
        ...
        function onWithdrawalSelect(withdrawalId){
            ...
            fetch('/withdraw/details/'+withdrawalId).then(r=>r.json()).then(data=>{
                ...
                returnSourceItems.forEach(it=>{
                    addReturnRowWithProduct(it.id, it.name, it.unit, it.maxQty, it.specs, it.category);
                });
        ...
        function addReturnRow(){
            ...
            tr.innerHTML=`
                <td><div class="custom-dropdown-wrap fx21-field"><input type="text" class="effect-21 return-item-search" placeholder="Select Product" autocomplete="off" required oninput="onReturnSearchInput(this)" onfocus="showReturnDropdown(this)" onblur="hideReturnDropdown(this)" style="width:100%;padding:6px;font-size:12px;border:1px solid #ccc;border-radius:4px;padding-right:26px;"><span class="focus-border"><i></i></span><span class="dd-caret">▾</span><input type="hidden" name="product_id[]" id="r-prod-${rowId}"><div class="custom-dropdown-list hidden"></div></div></td>
                <td><span class="readonly-cell r-spec">—</span></td>
                <td><span class="r-unit">—</span></td>
                <td><span class="r-cat">—</span></td>
                <td><div class="fx21-field"><select name="condition_status[]" id="r-cond-${rowId}" class="effect-21" required style="width:100%;padding:6px;font-size:12px;border:1px solid #ccc;border-radius:4px;background:#ffffff;"><option value="Serviceable">Serviceable / Unused</option><option value="Unserviceable">Unserviceable / Defective</option></select><span class="focus-border"><i></i></span></div></td>
                <td><span class="stock-info r-issued">—</span></td>
                <td><div class="fx21-field" style="display:inline-block;"><input type="number" name="returned_quantity[]" id="r-qty-${rowId}" class="effect-21" min="1" placeholder="0" style="width:90px; padding:6px; border:1.5px solid #d0dbe5; border-radius:6px;" oninput="validateReturnQty(${rowId})" required><span class="focus-border"><i></i></span></div></td>
        ...
        function addReturnRowWithProduct(productId, itemName, unit, issuedQty, specs, category){
            const tbody=document.getElementById('return-items-body');
            const rowId=tbody.rows.length;
            const tr=document.createElement('tr');
            tr.innerHTML=`
                <td><span style="font-weight:600;">${itemName}</span><input type="hidden" name="product_id[]" value="${productId}"></td>
                ...
                <td><div class="fx21-field" style="display:inline-block;"><input type="number" name="returned_quantity[]" id="r-qty-${rowId}" class="effect-21" min="1" max="${issuedQty}" placeholder="max ${issuedQty}" style="width:90px; padding:6px; border:1.5px solid #d0dbe5; border-radius:6px;" oninput="validateReturnQty(${rowId})" required><span class="focus-border"><i></i></span></div></td>
                <td><button type="button" class="btn-action btn-delete" title="Remove row" onclick="this.closest('tr').remove()"><svg class="act-icon" aria-hidden="true"><use href="#i-x"/></svg></button></td>
            `;
            tbody.appendChild(tr);
        }
